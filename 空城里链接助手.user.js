...
(function () {
    ...
    if(getValue('auth_code') == undefined || null){
    ...
    const _func = {
        _obj: {
            resolve(val) {
                ...
                let param = '';
                if (geetest_data == null || geetest_data == undefined || geetest_data == '') {
                    param = 'url=' + val;
                } else {
                ...
                if( document.domain == 'www.feimaoyun.com' && geetest_data == null || geetest_data == undefined || geetest_data == '' ){
                    // 滑动数据的简单验证
                ...
                GM_xmlhttpRequest({
                    method: "post",
                    url: 'http://resolve.ilzya.com:1306/resolve',
    ...
    if (window.location.host == 'www.feimaoyun.com'&& window.location.pathname.split("/")[1] == "s") {
        ...
        window.onload = function () {
            ...
            let nc_lang_cnt_loop = setInterval(function () {
                if(nc_lang_cnt&&nc_lang_cnt[0]&&nc_lang_cnt[0].outerText&&nc_lang_cnt[0].outerText=='请按住验证码提示，完成验证'){
