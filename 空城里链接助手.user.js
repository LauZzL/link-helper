...
(function () {
    ...
    'use strict';
    let feimao_actBox
    if (document.domain == 'www.feimaoyun.com') { // update 飞猫云 download (#0828 by @火锅)
        feimao_actBox = function (act4,act1,act2,act4_fn,act1_fn,act2_fn) {
            let actBox = document.getElementsByClassName('actBox')
            if(actBox&&actBox[0]){
                if(GM_getValue("act2_click")==0||GM_getValue("act2_click")==1){
                    act1 = act1?act1:"正在自动滑块，请稍等"
                }else{
                    act1 = act1?act1:"滑动滑块后自动解析"
                }
                act4 = "<div class='act act4'style='user-select:none'>"+(act4?act4:"微信搜索 IT空城里 关注不迷路！")+"</div>"
                act1 = "<div class='act act1'style='color:#466bff;user-select:none'>"+act1+"</div>"
                act2 = "<div class='actCodeBox act act2'style='text-align:center;user-select:none'>"+(act2?act2:"by 空城里@www.ilzya.com")+act2_clickfn()+"</div>"
                actBox[0].innerHTML = act4+act1+act2
                document.getElementsByClassName('act act4')[0].onclick = function () {
                    wd.style.display = wd.style.display == 'none' ? 'block' : 'none';
                }
                document.getElementsByClassName('act act4')[0].onclick =act4_fn?act4_fn:function () {
                    wd.style.display = wd.style.display == 'none' ? 'block' : 'none';
                }
                document.getElementsByClassName('act act1')[0].onclick =act1_fn?act1_fn:""
                document.getElementsByClassName('act act2')[0].onclick =act2_fn?act2_fn:function () {
                    document.getElementsByClassName('act act2')[0].innerHTML = "by 空城里@www.ilzya.com"+act2_clickfn(true)
                }
            }
        }
    }
    if(!GM_getValue("act2_click")){
        GM_setValue("act2_click",0)
    }
    let act2_clickfn = function (click_) {
        let list = {
            0:"<br>自动滑块",
            1:"<br>自动滑块后自动下载",
            2:"<br>手动滑块",
        ...
        let ac = GM_getValue("act2_click")?GM_getValue("act2_click"):0
        if(click_){
            ac = ac + 1 > 3 ? 0 : ac + 1
                        ...
                        if (result.code === 0) {
                            ...
                            if(feimao_actBox){
                                ...
                                if(GM_getValue("act2_click")==1||GM_getValue("act2_click")==3){//自动下载
                                    _func._obj.chrome_down(_global._durl,true,act4);
                                }//自动下载
                            }else{
                            ...
                            if(feimao_actBox){
                                ...
                                let act1 = "<font color=red>请刷新页面 "+result.message+"</font>"
                                feimao_actBox(act4,act1,"","",
                                              function(){
    ...
    if (document.domain == 'www.feimaoyun.com') {
            ...
            let class_remove = setInterval(function () {
                for (let i in classlist) {
                    ...
                    if(c && c[0]){
                        ...
                        clearInterval(class_remove)//停止循环
                    }
                }
            },30);
            //页面清理
